...
scroll_top(){ for _ in $(seq 1 10);do adb shell input swipe 540 650 540 1900 160||true;done;sleep 1; }
return_home(){ adb shell input keyevent 4;sleep 1;scroll_top;wait_for_text "COMMAND CENTER"; }

wait_for_text "COMMAND CENTER"
wait_for_text "RUNTIME"
wait_for_text "PASS"
# The fixture must be reachable from the APK before semantic checks begin.
wait_for_text "API ONLINE"
wait_for_text_with_scroll "CORE RULES"
scroll_top
wait_for_text "COMMAND CENTER"
wait_for_text "API ONLINE"

# Semantic E2E: real app -> HTTP fixture -> deterministic engines -> persistence -> rendered audit.
