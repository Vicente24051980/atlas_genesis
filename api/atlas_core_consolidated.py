...
ARCHITECTURE_VERSION = "ATLAS-E1-E6-PUBLIC-BOUNDARY-2026-09-07-v1"
...
def _sanitize_quant_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    API. E3/E4 and human approval remain separate downstream stages.
    """
    clean = dict(result)
...
async def monitor(
    ...
    return {
        ...
        "items": rows,
        "guardrail": "Monitoring produces E2 assessments only. E3 gates, portfolio selection, execution and human approval are separate stages.",
    }
...
async def engines() -> dict[str, Any]:
    return {
        ...
        "planeSeparation": "RESEARCH -> SIGNAL -> SCORE -> GATE -> PORTFOLIO_SELECTION -> EXECUTION",
        "guardrail": "Subordinate sensors are not independent canonical engines and cannot issue material actions.",
    }
