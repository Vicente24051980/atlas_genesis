...
# Historical N31 experiment only; ZERO current portfolio authority.
...
# Historical combo search intentionally restricted to the then-defined edge set.
...
def main():
    require_explicit_legacy_opt_in("scripts/atlas_structural_edge_combo_audit.py")
    ...
    out=Path(a.outdir); out.mkdir(parents=True,exist_ok=True); (out/'combo_audit.json').write_text(json.dumps({"status":"LEGACY_FIXED_PORTFOLIO_DIAGNOSTIC_ONLY","current_portfolio_authority":"NONE","base":base,"top":rows[:100]},indent=2),encoding='utf-8')
    lines=["# ATLAS Ω structural-edge combo diagnostic — LEGACY N31","","> LEGACY_FIXED_PORTFOLIO_DIAGNOSTIC_ONLY. ZERO current portfolio authority.","",f"Historical base U={base['u']:.4f} CAGR={base['cagr']:.2%} Vol={base['vol']:.2%} MaxDD={base['maxdd']:.2%}","","|#|k|OUT|IN|ΔU|CAGR|Vol|MaxDD|","|---:|---:|---|---|---:|---:|---:|---:|"]
