...
CHALLENGERS = ["WMS","CB","CBOE","RGLD","PWR","FIX","EME","WTW"]
BENCHMARK = "SPY"
TRADING_DAYS = 252
...
def calc_metrics(returns: pd.DataFrame, tickers: Sequence[str], spy_returns: pd.Series) -> PortfolioMetrics:
    ...
    # Research-only empirical risk proxy. It is NOT Permanent Loss Risk Ω.
...
def local_search_best_set(
    ...
    if n > len(candidates):
        raise ValueError("n exceeds candidate universe")
    current = [t for t in seed_order if t in candidates][:n]
    if len(current) < n:
        current += [t for t in candidates if t not in current][: n - len(current)]
...
def empirical_knee(frontier: Sequence[PortfolioMetrics], threshold: float = 0.0025) -> Tuple[int, str]:
    # First N where forward improvement is economically trivial (<25 bp utility proxy),
    # research-only and never canonical N*.
    ordered = sorted(frontier, key=lambda x: x.n)
    for a, b in zip(ordered, ordered[1:]):
        delta = b.empirical_utility_proxy - a.empirical_utility_proxy
        if delta < threshold:
            return a.n, f"delta_U_proxy_{a.n}_to_{b.n}={delta:.6f}<threshold={threshold:.6f}"
...
def write_report(outdir: Path, prices: pd.DataFrame, frontier: List[PortfolioMetrics], target_metrics: PortfolioMetrics, target_risk: Dict[str,float], knee: Tuple[int,str]) -> None:
    ...
    payload = {
        "status": "QUANT_DIAGNOSTIC_COMPLETE_EXPECTED_RETURN_EVIDENCE_PENDING",
        "methodology": {
            "selection_weights": "EQUAL_1_OVER_N_TEST_ONLY",
            "historical_return_is_expected_return": False,
            "global_optimality_proven": False,
            "search_mode": "DETERMINISTIC_ONE_SWAP_LOCAL_SEARCH",
            "empirical_risk_proxy": "0.65*abs(MaxDD)+0.20*abs(DailyCVaR95)*sqrt(252)+0.15*AnnualizedVol",
            "expected_return_state": "EXPECTED_RETURN_EVIDENCE_PENDING",
        },
        "sample": {"start": str(prices.index.min().date()), "end": str(prices.index.max().date()), "observations": int(prices.shape[0])},
        "target_31": asdict(target_metrics),
        "target_31_risk_contribution": target_risk,
        "empirical_frontier": [asdict(x) for x in frontier],
        "empirical_knee": {"n": knee[0], "reason": knee[1]},
    }
    (outdir / "quant_audit.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    lines = [
        "# ATLAS Ω Quantitative Portfolio Audit",
        "",
        "> Historical return is a diagnostic proxy, NOT Expected Return Ω. Canonical N* remains unresolved until forward ER inputs are complete.",
        "",
        f"Sample: {payload['sample']['start']} to {payload['sample']['end']} ({payload['sample']['observations']} daily observations)",
        "",
        "## Current target N=31 (equal-weight test exposure)",
        f"- Historical CAGR: {target_metrics.cagr:.2%}",
        f"- Annualized volatility: {target_metrics.annualized_vol:.2%}",
        f"- Max drawdown: {target_metrics.max_drawdown:.2%}",
        f"- Downside deviation: {target_metrics.downside_deviation:.2%}",
        f"- Beta vs SPY: {target_metrics.beta_vs_spy:.3f}",
        f"- Empirical utility proxy: {target_metrics.empirical_utility_proxy:.4f}",
        "",
        "## N=20..35 empirical frontier",
        "| N | CAGR | Vol | MaxDD | Beta | U_proxy |",
        "|---:|---:|---:|---:|---:|---:|",
    ]
    for m in sorted(frontier, key=lambda x: x.n):
        lines.append(f"| {m.n} | {m.cagr:.2%} | {m.annualized_vol:.2%} | {m.max_drawdown:.2%} | {m.beta_vs_spy:.3f} | {m.empirical_utility_proxy:.4f} |")
    lines += ["", f"Empirical knee diagnostic: **N={knee[0]}** ({knee[1]}).", "", "Expected Return Ω: **EVIDENCE_PENDING**."]
...
def main() -> None:
    ...
    ap.add_argument("--outdir", default="artifacts/quant-portfolio-audit")
    args = ap.parse_args()
    ...
    target = calc_metrics(stock_returns, TARGET_31, spy)
    target_rc = risk_contribution(stock_returns, TARGET_31)
    candidates = TARGET_31 + CHALLENGERS
    frontier = [local_search_best_set(stock_returns, candidates, n, spy, TARGET_31 + CHALLENGERS) for n in range(20,36)]
    knee = empirical_knee(frontier)
    write_report(Path(args.outdir), prices.drop(columns=[BENCHMARK]), frontier, target, target_rc, knee)
    print((Path(args.outdir) / "quant_audit.md").read_text(encoding="utf-8"))
