...
def write_report(outdir: Path, prices: pd.DataFrame, frontier: List[PortfolioMetrics], target_metrics: PortfolioMetrics, target_risk: Dict[str,float], knee: Tuple[int,str]) -> None:
    ...
    payload = {"status":"QUANT_DIAGNOSTIC_COMPLETE_EXPECTED_RETURN_EVIDENCE_PENDING","methodology":{"selection_weights":"EQUAL_1_OVER_N_TEST_ONLY","historical_return_is_expected_return":False,"global_optimality_proven":False,"search_mode":"DETERMINISTIC_ONE_SWAP_LOCAL_SEARCH","empirical_risk_proxy":"0.65*abs(MaxDD)+0.20*abs(DailyCVaR95)*sqrt(252)+0.15*AnnualizedVol","expected_return_state":"EXPECTED_RETURN_EVIDENCE_PENDING"},"sample":{"start":str(prices.index.min().date()),"end":str(prices.index.max().date()),"observations":int(prices.shape[0])},"target_31":asdict(target_metrics),"target_31_risk_contribution":target_risk,"best_empirical_31":asdict(best31),"empirical_frontier":[asdict(x) for x in frontier],"empirical_knee":{"n":knee[0],"reason":knee[1]}}
    (outdir / "quant_audit.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    lines=["# ATLAS Ω Quantitative Portfolio Audit","","> Historical return is a diagnostic proxy, NOT Expected Return Ω. Canonical N* remains unresolved until forward ER inputs are complete.","",f"Sample: {payload['sample']['start']} to {payload['sample']['end']} ({payload['sample']['observations']} daily observations)","","## Current target N=31 (equal-weight test exposure)",f"- Historical CAGR: {target_metrics.cagr:.2%}",f"- Annualized volatility: {target_metrics.annualized_vol:.2%}",f"- Max drawdown: {target_metrics.max_drawdown:.2%}",f"- Downside deviation: {target_metrics.downside_deviation:.2%}",f"- Beta vs SPY: {target_metrics.beta_vs_spy:.3f}",f"- Empirical utility proxy: {target_metrics.empirical_utility_proxy:.4f}","","## Best empirical N=31 challenger set",f"- Tickers: {', '.join(best31.tickers)}",f"- Historical CAGR: {best31.cagr:.2%}",f"- Volatility: {best31.annualized_vol:.2%}",f"- MaxDD: {best31.max_drawdown:.2%}",f"- U_proxy: {best31.empirical_utility_proxy:.4f}","","## N=20..35 empirical frontier","| N | CAGR | Vol | MaxDD | Beta | U_proxy |","|---:|---:|---:|---:|---:|---:|"]
