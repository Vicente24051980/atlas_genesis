...
export const MARKET_ENGINE_VERSION = 'ATLAS-MARKET-LAB-1.0.0';
...
export function marketSignals(rows) {
  if (rows.length < 22) {
    return {
      ...
      waveScore: null,
      downsideScore: null,
      downsideSeverity: 'UNKNOWN',
      streak: streakStats(rows),
      metrics: {},
      reasons: ['At least 22 daily observations are required.'],
...
export function discoveryScore(signals) {
  ...
  return Math.round(clamp(
    (signals.momentumScore || 0) * 0.45 +
    (signals.waveScore || 0) * 0.40 +
    (100 - (signals.downsideScore || 0)) * 0.15,
  ) * 10) / 10;
}
